...
{
    public partial class PythonForm : Form
    {
        ...
        public void FindPython()
        {
            string xmlFile = Application.StartupPath + "\\data\\Python.xml";

            DataSet ds = new DataSet();
            ds.ReadXml(xmlFile, XmlReadMode.InferSchema);

            DataTable PythonTables = ds.Tables["Python"];
            ...
            DataRow[] filteredRows = PythonTables.Select("Sourcecode_python LIKE '%" + curItem + "%'");
            List<string> nList = new List<string>();

            int i = 0;
            for (i = 0; i < filteredRows.Length; i++)
            {                
                string str = (filteredRows[i]["Sourcecode_python"].ToString());
        ...
        private void searchBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please in put your Contents");
            }
            else
            {
                string xmlFile = Application.StartupPath + "\\data\\Python.xml";
                ...
                DataTable CSharpTables = ds.Tables["Python"];

                string searchstring = textBox1.Text;

                DataRow[] filteredRows = CSharpTables.Select("Content_python LIKE '%" + searchstring + "%'");
                List<string> nList = new List<string>();
                ...
                try
                {
                    ...
                    for (i = 0; i < filteredRows.Length; i++)
                    {
                        ...
                        nList.Add(filteredRows[i]["Content_python"].ToString());
                    }
                ...
                {
                    MessageBox.Show(ex.Message, "Content Not Found!", MessageBoxButtons.AbortRetryIgnore);
                }
            }
        }
        ...
        public void xmldataPython()
        {
            ...
            for (i = 0; i <= ds.Tables["Python"].Rows.Count - 1; i++)
            {
                //nList.Add(ds.Tables["CSharp"].Rows[i].ItemArray[0] + " -- " + ds.Tables["CSharp"].Rows[i].ItemArray[1]);
                nList.Add("" + ds.Tables["Python"].Rows[i].ItemArray[1]);
            }
